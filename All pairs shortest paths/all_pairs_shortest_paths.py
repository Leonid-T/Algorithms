'''
Процедура all_pairs_shortest_paths решает задачу поиска кратчайших путей между
всеми парами вершин с помощью многократного умножения матриц.
'''
...
def all_pairs_shortest_paths(graph: Graph) -> None:
    W = get_matrix(graph)
    n = len(W)
    L = [m.copy() for m in W]
    m = 1
    while m < n - 1:
        ...
        m *= 2
    P = get_parents(L, W)
    print_paths(L, P, graph)


def extend_shortest_paths(L: List, W: List) -> List:
    n = len(L)
    M = [[float('inf') for _ in range(n)] for _ in range(n)]
    for i in range(n):
        for j in range(n):
            for k in range(n):
                if M[i][j] > L[i][k] + W[k][j]:
                    M[i][j] = L[i][k] + W[k][j]
    return M
...
def get_parents(L: List, W: List) -> List:
    n = len(L)
    P = [[None for j in range(n)] for i in range(n)]
    for i in range(n):
        for j in range(n):
            for k in range(n):
                if L[i][j] == L[i][k] + W[k][j] and k != j:
                    P[i][j] = k
    return P
