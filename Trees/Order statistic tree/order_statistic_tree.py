'''
Дерево порядковой статистики является расширением красно-черного дерева, при
котором в каждом узле дополнительно хранится размер поддерева. Таким образом,
добавлена поддержка операций порядковой статистики: поиск элемента,
находящегося i-ым в порядке возрастания (select); определение позиции элемента
с ключем x в порядке возрастания (rank).
'''
...
def main():
    ...
    print(tree)
    num = 5
    print(f'{tree.select(num)} is {num} element of OSTree')
    key = 12
    print(f'{tree.search(key)} is {tree.rank(key)} element of OSTree')
